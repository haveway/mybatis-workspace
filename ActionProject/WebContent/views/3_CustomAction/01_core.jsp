...
<body>
	...
	<pre>
		* 변수 선언(&lt;c:set var="변수명" value="리터럴" scope="스코프영역지정(생략가능)"/&gt;)
		...
		- 더나아가서 해당 변수를 어떤 socpe영엑에 담아둘건지 지정 가능함
			(scope속성 생략 시 pageScope에 담김)
		=> 즉, 해당 scope영역에 setAttribute라는 메소드를 이용해서
			key + value형태로 데이터를 담아둔다라고 생각하면 된다.
		=> c:set을 통해 선언된 변수는 EL로 접근해서 사용 가능(스크립팅 원소로는 접근 불가능)
		
		* 주의사항
		- 변수의 타입을 별도로 지정하지 않음(초기값을 반드시 지정해야둬야됨!!!)
	</pre>
	...
	<!-- 
		...
		: 티가 나지는 않지만 속도가 느려질수도있따.(스코프영역.변수명 을 권장)
	 -->
	 ${ pageScope.num1 } <br>
	 ...
	 outTest를 c:out으로 출력할건데 escapeXml속성을 생략하고 출력(true) : <c:out value="${ outTest }" />
	 ...
	 <pre>
	 	...
	 	&lt;c:choose&gt;
			...
			...	 
	 		&lt;c:otherwise&gt; ~~~~ &lt;/c:otherwise&gt; => else블럭에 해당됨, switch문의 default
	 	&lt;/c:choose&gt;
	 ...
	 <%
	 	...
	 	//여기까지 코드가 원래 이페이지가 응답되기 전에 서블릿에서 완성했어야 하는코드
	 %>
